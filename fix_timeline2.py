...
old_timeline = """                var virtualDurationMs = durationMs
...
# A small trick, the indentation in the file might be different so I will use re
old_timeline_pattern = re.compile(r'var virtualDurationMs = durationMs\s+var virtualPositionMs = currentPositionMs\s+if \(true\) \{ // Always show virtual timeline.*?\}\s+\}', re.DOTALL)
content = old_timeline_pattern.sub(new_timeline, content)
